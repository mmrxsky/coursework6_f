...
class Client(models.Model):
    ...
    email = models.EmailField(
        max_length=100, verbose_name="Контактный email", unique=True
    )
    comment = models.TextField(verbose_name="Комментарий", **NULLABLE)

    owner = models.ForeignKey(
        User, verbose_name="Менеджер", **NULLABLE, on_delete=models.SET_NULL
    )
    count = models.PositiveIntegerField(
        verbose_name="Количество клиентов рассылок", default=0
    )
...
class NewsLetter(models.Model):
    ...
    client = models.ManyToManyField(
        Client, verbose_name="Клиент сервиса", help_text="Укажите клиентов"
    )
    message = models.ForeignKey(
        Message, verbose_name="Сообщение", on_delete=models.CASCADE, **NULLABLE
    )
    ...
    count = models.PositiveIntegerField(verbose_name="Количество рассылок", default=0)
...
class Attempt(models.Model):
    ...
    status = models.CharField(
        verbose_name="статус попытки (успешно / не успешно)", choices=STATUS_VARIANTS
    )
